...
| **S4** Sitzungswissen-Gerüst | **nur wenn das aktuelle Repo eine eigene Wissensbasis führt** (im OS-Repo: `knowledge-base/`) — dort unter `sitzungswissen/` mit den Bausteinen `roll-up.md` · `offene-straenge-register.md` · je Abteilung `<abteilung>/stand.md` · `<abteilung>/journal/`; **ohne eigene Wissensbasis (Regelfall bei Kunden-/Fremd-Repos): nicht anwendbar** — dort legt der Kern nichts mehr an, das Projekt-Memory trägt den Stand allein (kein Dateistrom; der frühere Strom `.nc/erinnerung/` ist abgeschafft) | Existenz-Check je Baustein bei eigener Wissensbasis; sonst Befund „kein Dateistrom" |
