---
name: attribution-spec
description: Attribution-Logik im Empfehlungssystem prüfen/bauen — Normalisierung, hart/fuzzy Matching, Zeitfenster, Mehrfach-Match-Guard, Tie-Break. nutzen VOR und WÄHREND Arbeit an `lib/attribution/` oder `lib/normalize/` (Phase 2). Quelle: Projektplan v2.3 §4/§5, CLAUDE.md §5.
---

# Attribution-Spezifikation (Geld-Pfad)

Zweck: Jede Änderung an Matching/Normalisierung muss diese Checkliste passieren. Fakten **nur** aus `Dokumente/Projektplan Empfehlungssystem v2.md` §4/§5 und `CLAUDE.md` §5 — niemals aus dem Gedächtnis.
...
- [ ] Refund/Storno vor Ablauf → Bedingung erlischt (siehe Skill `reward-guard`).
