...
| 5 | `plugins/nc/module-registry.json` + `vorlagen/abteilungsplugin/VORLAGE.md` (repo-intern) bzw. eigene `module-registry.json` des Satelliten (`abteilungs-plugin-bau.md` §3b) | Schema, Status, Kriterien der Abteilungs-/Modul-Metadaten | `referenz/pflege-auspraegung.md` ist in NC **noch nicht** gebaut (geplant Phase E) — sobald vorhanden, hier ergänzen, nicht vorwegnehmen |
