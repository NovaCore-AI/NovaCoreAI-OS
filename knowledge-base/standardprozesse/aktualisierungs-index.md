...
| **Agent (Subagent) neu/geändert** (`agents/`-Verzeichnis eines Plugins) | [`subagenten-bau.md`](subagenten-bau.md) (Faustregel Agent-vs-Skill, Scope, Gate-Semantik) · `plugins/nc/referenz/agent-authoring.md` (Feld-Kanon, Werkzeuggrenzen-Regel inkl. **Diagnose-Klasse** seit 1.4.3) | Agent-Datei · Registry-`agents`-Segment · README (Agenten-Nennung + Overlap-Matrix) · `AGENTS.md` (Repo-Karte) · Trigger-/Overlap-Prüfung gegen Agents UND Skills · `CHANGELOG.md` · Kern-Bump (bzw. Bump des tragenden Plugins) | **Werkzeuggrenze hart im Frontmatter** (Allowlist-Prinzip seit 2026-08-15: `tools` + `model` Pflichtfelder; read-only = Allowlist ohne Schreib-Tools und ohne `Bash`; **Diagnose** = Marker `<!-- nc:diagnose -->` + genau `Bash` mit benannter, lesender Kommando-Allowlist im Prompt, Grenze in der `description` sichtbar, `maxTurns` Pflicht — seit Prüfbaustein 1.4.3/Phase J; schreibend nur mit Marker + begründeter Schreib-Allowlist ohne `Bash`; schreibend und diagnose schließen sich aus) · Defense-Baseline-Block Pflicht · verbotene Felder `hooks`/`mcpServers`/`permissionMode` · Praxistest mit **Negativprobe** · portabler Prüfbaustein (`agenten.test.mjs`, Baustein-Version im Kopf) wandert bei Satelliten-Extraktion mit |
...
3. **Parallele Arbeitsstränge und Anker:** **Versionen werden nicht mehr reserviert** — sie
   ...
   sowie für Nachtrags-/AP-Kennungen gilt seit Phase J (D31, 2026-08-25) allein die
   **Mechanik** statt einer Reservierungsnorm (Begriffsnorm „Anker" als Absatz in
   [`os-bau-methode.md`](os-bau-methode.md)): Merge-Konflikt bei Dateikollision ·
   Suite-Invariante gegen doppelte CHANGELOG-Versionsüberschriften · Pflichtschritt
   „fremde Worktrees vor dem ersten Schreiben prüfen". **Es wird nichts reserviert** —
   Reserve-/Anker-Tags als Git-Ref sind entfallen (P-E2), Tags entstehen ausschließlich
   als Teil eines Release.
