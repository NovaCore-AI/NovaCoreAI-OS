...
| 6 | **Kern-Entwurf + Protokoll** | Agent | **Sichtbarkeitsmodell:** „Kern verdichtet und verweist, Abteilung dokumentiert vollständig" — der Kern-Beitrag ist ein **konzentriertes Fakten-Dokument** (Datum, Herkunftsabteilung, auflösbare Fundstelle in der Abteilungsquelle), nie eine Volltext-Kopie und nie ein Umzug; fehlt die auflösbare Fundstelle, wird die Zeile zurückgestellt statt geraten übernommen. Zielkategorie nach `SSOT-Document-Index` Teil 1, Index-Zeile Teil 2 ist Pflicht. Dazu die **committete Protokolldatei** in `queue-protokolle/` mit Entscheid, Begründung und Ziel-Dokumentpfad je geprüfter Zeile |
