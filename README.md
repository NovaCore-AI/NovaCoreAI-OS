...
|---|---|---|---|
| `nc` | **Kern** — ständige Abteilung `gemeinsam`: Session-Zyklus, Infrapflege-Skills, SSOT-Präsenz-Router (`wissen-*`), Queue-Flow-Skills (`queue-abteilung`/`queue-kern`), Kontroll-Schicht (Gate 1 + Gate 2 + Gate 3 + Queue-Fälligkeits-**Anweisung** + Setup-Hinweis + zwei Zeiger-Hooks), Doks-Autosync, WP-Rahmen, Registry, Formatregeln, `doks/nc-teamsync.md`, Subagenten (`agents/`) | `/nc:` | 0.15.1 (= `VERSION`) |
| `nc-development` | Abteilung development — Module `fe` / `be` / `flc` / `wzs` / `qs` / `rel` plus 3 Subagenten (`code-reviewer`, `pipeline-praeflight`, `test-luecken-scout` — siehe [Subagenten der Abteilung](#subagenten-der-abteilung-development)) | `/nc-development:` | 0.3.0 |
...
### Subagenten der Abteilung development

Drei isolierte Subagenten (Onsite-Delta D19, 2026-08-26; Overlap-Matrix in
[`plugins/nc-development/README.md`](plugins/nc-development/README.md)):

- **`code-reviewer`** (`model: inherit`, read-only) — isoliertes 4-Augen-Review eines Diffs
  mit den Skills `fe-review`/`be-review`; Findings als BLOCKER/MAJOR/MINOR/NIT-**Entwurf**.
- **`pipeline-praeflight`** (`model: sonnet`, **Diagnose-Klasse** `<!-- nc:diagnose -->`) —
  stellt die `ci.yml`-Schritte lokal nach (Suite + beide Validierungen) und liefert eine
  belegte Grün-Prognose; nur nicht-mutierende Kommandos einer festen Allowlist, im
  Sekundärmodus read-only `gh run`-Analyse roter Läufe. Kein `mcp__*`.
- **`test-luecken-scout`** (`model: sonnet`, read-only) — priorisierte Testgerüst-Vorschläge
  nach Lückenklassen (keine Tests · Happy-Path-only · fehlende Fehlerpfade · Edge-Cases);
  schreibt keine Tests.
...
| `nc-safety-gate` (Gate 3, Safety-Gate) | PreToolUse (`Bash`, `mcp__.*`) | Echter Freigabedialog (`permissionDecision: "ask"`) vor riskanten Aktionen: Musterliste v1 im NovaCore-Zuschnitt (tofu/terraform apply/destroy, generisches deploy-Wort mit Verbpositions-Ausnahme get/describe/logs, mcp-Schreibverben über Werkzeug- und Parameternamen, **Muster 4 DB-Hälfte seit 2026-08-25**: Prisma-Schreibwege und `docker compose exec postgres psql` mit Lese-Ausnahmen als Fehlalarm-Schutz), inkl. der vier GLM-Bypass-Härtungen des Vorbilds (Shell-Wrapper-Rekursion, Präfix-Kommandos, gequotetes Kommandowort, Verbpositions-Schärfung). **Wertentscheidung bei `NAME=wert` (seit 2026-08-26, D33):** bewertet werden nur im quote-bereinigten Strom **real zugewiesene** Namen; der **Wert** entscheidet dreistufig — deploy-Wort im Wert fragt (`MODE=deploy-prod`), deploy-Wort im Namen mit Prod-Präfix (`prod*`/`prd*`/`live*`) oder statisch unauflösbarem Wert (`$VAR`, Substitution) fragt, jeder andere Wert bleibt still (`DEPLOYMENT_TYPE=dev`); leerer Wert ist kein Prod-Ziel. Kein State — jeder Treffer fragt erneut; Subagenten **nicht** ausgenommen. Opt-out `NC_SAFETY_GATE=off`. |
| `nc-doks-autosync` | SessionStart | Hält **zwei** Ziele unabhängig voneinander auf dem Stand des installierten Kerns: Ebene 1 — Firmen-Block in `~/.claude/CLAUDE.md` per Marker-Chirurgie (`NC:BLOCK:…`), **Privat-Zone außerhalb der Marker wird nie verändert**, bei defekten Markern wird nichts geschrieben; Ebene 1b — Team-Sync-Datei `~/.claude/nc-teamsync.md` als Ganzdatei mit Versions-Stempel in Zeile 1 (Payload: `doks/nc-teamsync.md`). Backup vor jedem Schreiben (intakte Sicherungen werden nie verschlechtert), atomarer Write, zeilenenden-normalisierter Identitätsvergleich (kein CRLF-Churn). Opt-out `NC_AUTOSYNC=off` (beide Ziele), Test-Overrides `NC_AUTOSYNC_TARGET` / `NC_AUTOSYNC_TEAMSYNC_TARGET`. |
...
| `nc-queue-faelligkeit` (Queue-Fälligkeits-**Anweisung**, **kein Gate**) | SessionStart | **Weist die Session an, den fälligen Lauf JETZT auszuführen** (seit 2026-08-26, D28): Titel „Queue-Flow fällig: JETZT ausführen (keine Blockade)", Anweisung an die Session — Skill selbst ausführen oder Subagenten beauftragen, erster Schritt nach aktueller Arbeit, rote Linie bis zum fertigen PR (Merge bleibt Mensch), Stempel-Kommando im Text; früher reine Erinnerung. Meldet zwei Fälligkeiten des Queue-Flows: nicht eingereichte Wissensbasis-Arbeit im Abteilungs-Satelliten-Klon und offene Zeilen der **gemergten** Abteilungs-Queue — 14-Tage-Takt plus ein Tag Versatz. Blockiert nichts, injiziert nur die Anweisung; höchstens fünf lokale Git-Aufrufe für den Fälligkeits-Teil. Repo-Pfade ausschließlich über die Infra-Registry `~/.claude/nc/infra.json`; fehlt dort ein Abteilungs-Satellit, schweigt der Hook (heutiger Übergangszustand — `development` liegt repo-intern). Zweiter Befund desselben Hooks: PR-Sichtbarkeit über alle Repos der Infra-Registry — der einzige, mehrfach gedeckelte Netzzugriff der Kontroll-Schicht (Zeitbudget, Tages-Cache, Schweigen bei unbrauchbarem `gh`). Subagenten ausgenommen. Opt-out `NC_QUEUE_CHECK=off` (ganzer Hook), `NC_PR_CHECK=off` (nur der PR-Netzteil), Test-Overrides `NC_QUEUE_STATE_DIR` / `NC_QUEUE_SESSION_DIR` / `NC_QUEUE_PFAD` / `NC_PR_CMD`. |
| `nc-setup-hinweis` (Setup-Beleg-Zeiger, **kein Gate**) | SessionStart | Prüft den init-Beleg der Infra-Registry `~/.claude/nc/infra.json` und weist bei Bedarf zur Instandsetzung an — vier Zustände, erster zutreffender gewinnt: **fehlt** (Registry nicht da → `/nc:setup` aufrufen) · **neuer** (höhere `schemaVersion` → Marketplace-Update, nicht setup) · **defekt** (toter `kernRepoPfad`, fehlendes Pflichtfeld → Grund nennen) · **grün** (stumm, null Ausgabe). Liest nur über die gemeinsame Lib `hooks/lib/infra-registry.js` (Invariante: ein Beleg, ein Schreiber — `infra.json` schreibt ausschließlich `/nc:setup`); Pfad-Liveness kennt Worktrees (`.git` als Datei) und Windows-8.3-Kurzpfade. Anweisung höchstens einmal je Sitzung (Marker in `os.tmpdir()`, atomarer Write, defekter Marker = schweigen), Text-Reihenfolge `/nc:start` → Stempel → `/nc:setup`; **kein Netz, kein Gate, fail-open**. Subagenten ausgenommen. Opt-out `NC_SETUP_HINWEIS=off`, Test-Overrides `NC_SETUP_STATE_DIR` / `NC_SETUP_SESSION_DIR`. |
...
riskanten Aktionen (Musterliste v1 im NovaCore-Zuschnitt, Wortlaut-Abnahme erfolgte am
Phase-G-PR; die `NAME=wert`-Wertentscheidung Phase J am Phase-J-PR). Gate 4 (Sitzungsabschluss
als eigener Hook) ist **endgültig entfallen** (Onsite
